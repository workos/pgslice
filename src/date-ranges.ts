...
export function parsePartitionDate(
  ...
  switch (period) {
    ...
    case "week": {
      // Format: <ISO year>w<ISO week>, e.g. "2026w32"
...
/**
 * Parses the day component of a single RANGE bound value (e.g. the text inside
 * `FROM (...)` / `TO (...)`). Partition boundaries for time-range tables are
 * day-aligned, so we read the `YYYY-MM-DD` and build a UTC date — this avoids
 * the timezone ambiguity of parsing a bare `timestamp without time zone`
 * literal, and is exact for `date`, `timestamp`, and `timestamptz` keys alike.
 */
function parseBoundValue(value: string): {
  ...
  if (!match) {
    return { date: null, unbounded: false };
  }
...
export function* extendRangesBackward(options: {
  ...
  for (
    let count = 0;
    advanceDate(end, options.period, -1).getTime() >=
      options.horizon.getTime() && count < MAX_GENERATED_PARTITIONS;
    count++
  ) {
