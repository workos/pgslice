...
describe("Pgslice.addPartitions (retrofit)", () => {
  ...
  describe("extending an existing partitioned table (by bounds, no rename)", () => {
    async function boundsByName(transaction: DatabaseTransactionConnection) {
      await transaction.query(sql.unsafe`SET TIME ZONE 'UTC'`);
    ...
    test("extends a year-resetting weekly scheme across the year boundary with no gap or overlap", async ({
      ...
      // No two partitions overlap, and coverage is contiguous.
      const ranges = rows
        .map((r) => {
          const match = r.bound.match(/FROM \('(.+?)'\) TO \('(.+?)'\)/);
          return {
            lo: new Date(`${match?.[1] ?? ""}Z`).getTime(),
            hi: new Date(`${match?.[2] ?? ""}Z`).getTime(),
          };
        })
        .sort((a, b) => a.lo - b.lo);
      for (let i = 1; i < ranges.length; i++) {
        expect(ranges[i].lo).toBe(ranges[i - 1].hi);
      }
    });
    ...
    test("aligns new partitions to UTC regardless of the session timezone", async ({
      ...
      await transaction.query(sql.unsafe`SET TIME ZONE 'America/New_York'`);
      await transaction.query(sql.unsafe`
        CREATE TABLE evt (
          id varchar NOT NULL,
          created_at timestamp with time zone NOT NULL,
